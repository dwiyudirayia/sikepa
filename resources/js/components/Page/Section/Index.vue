        ...
        <div class="m-portlet m-portlet--creative m-portlet--bordered-semi">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <router-link to="/section/page/create" class="btn m-btn btn-success btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Tambah Section Page'">
                        <span>
                            ...
                            <div class="m-widget5__item" v-for="value in this.$store.getters['page/getData']" :key="value.id">
                                ...
                                <div class="m-widget5__content">
                                    <div class="m-widget5__stats1">
                                        <router-link :to="{name: 'ListSectionCategoryPage', params: { id: value.id }}" class="btn m-btn btn-primary btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Melihat Daftar Section Kategori'">
                                            <span>
                                                <i class="la la-list"></i>
                                                <span>Daftar Kategori</span>
                                            </span>
                                        </router-link>
                                        <router-link :to="{name: 'SectionPageEdit', params: { id: value.id }}" class="btn m-btn btn-success btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Untuk Edit Section'">
                                            <span>
...
export default {
    ...
    computed: {
        ...
        {
            return this.$store.getters['page/getShowNotificationLoginSuccess'];
        }
    },
    created() {
        this.$store.dispatch('page/indexSection');
    },
    methods: {
        destroy(id) {
            this.$store.dispatch('page/destroySection', id);
        },
        confirmDelete(id)
        {
            Swal.fire({
