                                            ...
                                            <tr v-for="(value, index) in youSubmission" :key="value.id">
                                                ...
                                                <td>
                                                    ...
                                                    <template v-else-if="value.status_disposition < 9 || value.status_disposition > 9">
                                                        <router-link v-if="$can('Bagian Kerjasama') == false" :to="{name: 'PKSProposalSubmissionCooperationDetail', params: { id: value.id }}" class="btn m-btn btn-success btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Untuk Detail Pengajuan'">
                                                            <span>
                                                                <i class="la la-eye"></i>
                                                                <span>Detail Pengajuan</span>
                                                            </span>
                                                        </router-link>
                                                        <router-link v-if="$can('Bagian Kerjasama') && value.status_disposition != 9" :to="{name: 'PKSProposalSubmissionCooperationYourDetailPreview', params: { id: value.id }}" class="btn m-btn btn-brand btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Untuk Detail Pengajuan'">
                                                            <span>
