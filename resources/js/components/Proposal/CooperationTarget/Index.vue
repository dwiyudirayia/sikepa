        ...
        <div class="m-portlet m-portlet--creative m-portlet--bordered-semi">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <router-link to="/proposal/cooperation/target/create" class="btn m-btn btn-success btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Tambah Target Kerjasama Proposal'">
                        <span>
                            <i class="la la-plus"></i>
                            <span>Tambah Target Kerjasama</span>
                        </span>
                    </router-link>
                </div>
            </div>
                            ...
                            <div class="m-widget5__item" v-for="value in this.$store.getters['proposal/getData']" :key="value.id">
                                ...
                                <div class="m-widget5__content">
                                    <div class="m-widget5__stats1">
                                        <router-link :to="{name: 'ProposalCooperationTargetEdit', params: { id: value.id }}" class="btn m-btn btn-success btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Untuk Edit Target Kerjasama Proposal'">
                                            <span>
                                                <i class="la la-pencil"></i>
                                                <span>Edit Target Kerjasama</span>
                                            </span>
                                        </router-link>
                                        <button @click="confirmDelete(value.id)" class="btn m-btn btn-danger btn-sm  m-btn--icon m-btn--pill icon-only" v-tooltip.top="'Untuk Hapus Target Kerjasama Proposal'">
                                            <span>
                                                <i class="la la-trash"></i>
                                                <span>Hapus Target Kerjasama</span>
                                            </span>
                                        </button>
                                    </div>
                                </div>
...
export default {
    ...
    created() {
        this.$store.dispatch('proposal/indexCooperationTarget');
    },
    methods: {
        destroy(id) {
            this.$store.dispatch('proposal/destroyCooperationTarget', id);
        },
        confirmDelete(id)
        {
            Swal.fire({
