...
export default {
    name: 'ExtensionMonevActivityP3Edit',
    data() {
        return {
            forms: {
                title_activity: null,
            ...
            breadcrumbLink: [
                ...
                },
                {
                    ...
                    path: `/monev/extension/detail/${this.$route.params.id}/satker/edit`
                },
            ],
        }
    },
    ...
    methods: {
        storeFile() {
            ...
            $axios.post(`/admin/extension/store/file/activity/satker`, formData)
            .then(response => {
                toastr.options = {
                    "closeButton": false,
                    "debug": false,
        ...
        downloadImage(id) {
            window.location.href = `/api/admin/extension/download/image/activity/satker/${id}?token=${localStorage.getItem('token')}`;
        },
        hapusImage(id) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $axios.delete(`/admin/extension/image/activity/satker/${id}`)
                    .then(response => {
                        this.getData();
                    })
                }
        ...
        getData() {
            $axios.get(`/admin/extension/monev/activity/satker/${this.$route.params.id}/edit`)
            .then(response => {
        ...
        update() {
            ...
            $axios.put(`/admin/extension/monev/activity/satker/${this.$route.params.id}`, this.forms)
            .then(response => {
                toastr.options = {
                    "closeButton": false,
                    "debug": false,
