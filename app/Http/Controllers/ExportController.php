...
class ExportController extends Controller
{
    ...
    public function downloadFormatMOUWordAdendum($id)
    {
        $phpWord = new \PhpOffice\PhpWord\PhpWord();
        $proposal = Adendum::find($id);
        $getIdProposal = $proposal->id;
        $getMailingNumber = $proposal->mailing_number;
        $getAgencyName = $proposal->agency_name;
        $section = $phpWord->addSection();
        ...
        $phpWord->addFontStyle('headerFontStyle', array('size'=>12, 'name' => 'Bookman Old Style'));
        $phpWord->addParagraphStyle('headerParagraphStyle', array('align'=>'center', 'spaceAfter'=>200));
        $section->addImage(storage_path("app/public/barcode_adendum/$getIdProposal/barcode-$getMailingNumber.png"),['positioning' => 'absolute', 'width' => 60, 'height' => 60]);
        $section->addText("KESEPAKATAN BERSAMA","headerFontStyle","headerParagraphStyle");
        $section->addText("ANTARA","headerFontStyle","headerParagraphStyle");
        $section->addText("$getAgencyName","headerFontStyle","headerParagraphStyle");
        $section->addText("DENGAN","headerFontStyle","headerParagraphStyle");
    ...
    public function downloadFormatMOUWordGuestAdendum($id)
    {
        $phpWord = new \PhpOffice\PhpWord\PhpWord();
        $proposal = AdendumGuest::find($id);
        $getIdProposal = $proposal->id;
        $getMailingNumber = $proposal->mailing_number;
        $getAgencyName = $proposal->agency_name;
        $section = $phpWord->addSection();
        ...
        $phpWord->addFontStyle('headerFontStyle', array('size'=>12, 'name' => 'Bookman Old Style'));
        $phpWord->addParagraphStyle('headerParagraphStyle', array('align'=>'center', 'spaceAfter'=>200));
        $section->addImage(storage_path("app/public/barcode_guest_adendum/$getIdProposal/barcode-$getMailingNumber.png"),['positioning' => 'absolute', 'width' => 60, 'height' => 60]);
        $section->addText("KESEPAKATAN BERSAMA","headerFontStyle","headerParagraphStyle");
        $section->addText("ANTARA","headerFontStyle","headerParagraphStyle");
        $section->addText("$getAgencyName","headerFontStyle","headerParagraphStyle");
        $section->addText("DENGAN","headerFontStyle","headerParagraphStyle");
    ...
    public function downloadSummaryGuest($id) {
        $data = SubmissionProposalGuest::with('deputi.role', 'country','agencies', 'typeOfCooperationOne', 'typeOfCooperationTwo')->findOrFail($id);
        $pdf = PDF::loadView('export.summary-proposal-guest', compact('data'));
        return $pdf->download('Rangkuman Jenis Kerjasama '.date('Y-m-d_H-i-s').'.pdf');
    }
    public function downloadSummaryAdendum($id) {
        $data = Adendum::with('deputi.role', 'country','agencies', 'typeOfCooperationOne', 'typeOfCooperationTwo')->findOrFail($id);
        $pdf = PDF::loadView('export.summary-proposal', compact('data'));
        return $pdf->download('Rangkuman Jenis Kerjasama '.date('Y-m-d_H-i-s').'.pdf');
    }
    public function downloadSummaryGuestAdendum($id) {
        $data = AdendumGuest::with('deputi.role', 'country','agencies', 'typeOfCooperationOne', 'typeOfCooperationTwo')->findOrFail($id);
        $pdf = PDF::loadView('export.summary-proposal-guest', compact('data'));
        return $pdf->download('Rangkuman Jenis Kerjasama '.date('Y-m-d_H-i-s').'.pdf');
    }
    public function downloadSummaryExtension($id) {
        $data = Extension::with('deputi.role', 'country','agencies', 'typeOfCooperationOne', 'typeOfCooperationTwo')->findOrFail($id);
        $pdf = PDF::loadView('export.summary-proposal', compact('data'));
        return $pdf->download('Rangkuman Jenis Kerjasama '.date('Y-m-d_H-i-s').'.pdf');
    }
    public function downloadSummaryGuestExtension($id) {
        $data = ExtensionGuest::with('deputi.role', 'country','agencies', 'typeOfCooperationOne', 'typeOfCooperationTwo')->findOrFail($id);
        $pdf = PDF::loadView('export.summary-proposal-guest', compact('data'));
        return $pdf->download('Rangkuman Jenis Kerjasama '.date('Y-m-d_H-i-s').'.pdf');
    }
}
